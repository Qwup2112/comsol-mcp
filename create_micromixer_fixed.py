...
import os
# Set COMSOL installation path for mph discovery
...
# Patch mph discovery to recognize our COMSOL installation
...
def patched_backend(version=None):
    if version is None:
        version = '6.2'
    print(f"DEBUG: patched_backend called with version={version}")
    try:
        result = original_backend(version)
        print(f"DEBUG: original_backend succeeded")
        return result
    except RuntimeError as e:
        print(f"DEBUG: original_backend failed: {e}")
        print(f"DEBUG: Creating synthetic backend for D:\\COMSOL62\\Multiphysics")
        # If registry lookup fails, try direct path
        # Create a Backend dict with required keys
        # Note: 'server' must be a list for concatenation in Server.__init__
        backend = {
            'name': 'COMSOL',
            'major': 6,
            'minor': 2, 
        ...
        print(f"DEBUG: returning synthetic backend: {backend}")
        return backend
...
try:
    ...
    # Try importing session directly for more control
    from mph import session, server
    srv = server.Server(cores=4, version="6.2", timeout=300)  # 5 minute timeout
    client = session.Session(srv)
    print("    Client started with extended timeout!")
